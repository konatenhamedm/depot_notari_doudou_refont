...
class WorkflowController extends BaseController
{
    ...
    #[Route('/', name: 'app_parametre_workflow_index', methods: ['GET', 'POST'], options: ['expose' => true])]
    public function index(Request $request, DataTableFactory $dataTableFactory): Response
    {
        ...
        $table = $dataTableFactory->create()
            ...
            ->add('libelleEtape', TextColumn::class, ['label' => 'Libelle Etape'])
            ->add('propriete', TextColumn::class, ['label' => 'Propriété'])
            ->add('NombreJours', TextColumn::class, ['label' => 'Nombre Jours', 'className' => 'text-center w-150px'])
            ->add('type', TextColumn::class, ['label' => 'Type acte', 'field' => 'type.titre'])

            ->createAdapter(ORMAdapter::class, [
                'entity' => Workflow::class,
                'query' => function (QueryBuilder $qb) use ($type) {
                    $qb->select(['p', 'type'])
                        ->from(Workflow::class, 'p')
                        ->join('p.typeActe', 'type');
            ...
            ->setName('dt_app_parametre_workflow_' . $type);
        if ($permission != null) {
            ...
            if ($hasActions) {
                $table->add('id', TextColumn::class, [
                    'label' => 'Actions', 'orderable' => false, 'globalSearchable' => false, 'className' => 'grid_row_actions', 'render' => function ($value, Workflow $context) use ($renders) {
                        $options = [
                            ...
                            'actions' => [
                                'edit' => [
                                    ...
                                    'url' => $this->generateUrl('app_parametre_workflow_edit', ['id' => $context->getGestionWorkflow()->getId()]), 'ajax' => true, 'icon' => '%icon% bi bi-pen', 'attrs' => ['class' => 'btn-default'], 'render' => $renders['edit']
                                ],
                                'show' => [
